...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    joint_p = 1
    for i in people:
        if i in one_gene: pgene = 1
        elif i in two_genes: pgene = 2
        else: pgene = 0

        if i in have_trait: ptrait = True
        else: ptrait = False

        if not people[i]['mother'] and not people[i]['father']:
            if pgene == 1:
                joint_p *= PROBS["gene"][1] * PROBS["trait"][pgene][ptrait]
            elif pgene == 2:
            ...
            if pgene == 2:
                joint_p *= (mother_prob * father_prob
                            * PROBS["trait"][pgene][ptrait])
            elif pgene == 1:
                joint_p *= ((1 - mother_prob) * father_prob
                            + (1 - father_prob) * mother_prob
                            * PROBS["trait"][pgene][ptrait])
            else:
                joint_p *= (mother_prob * father_prob * PROBS["trait"][pgene][ptrait]
                            + (1 - mother_prob) * (1 - father_prob)
                            * PROBS["trait"][pgene][ptrait])
    return joint_p
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for i in probabilities:
        ...
        probabilities[i]["gene"][pgene] += p
        probabilities[i]["trait"][ptrait] += p
...
def normalize(probabilities):
    ...
    """
    for i in probabilities:
        genesum = sum(probabilities[i]["gene"].values())
        traitsum = sum(probabilities[i]["trait"].values())
        for j in probabilities[i]["gene"]:
