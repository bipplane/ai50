...
def get_model():
    ...
    model = tf.keras.models.Sequential([
        tf.keras.layers.Conv2D(     # 1st layer, 32 filters of size 3x3
            32, (3, 3), activation="relu", input_shape=(IMG_WIDTH, IMG_HEIGHT, 3)
        ),
        tf.keras.layers.MaxPooling2D(pool_size=(2, 2)),    # 2x2 max pooling
        tf.keras.layers.Conv2D(       # 2nd layer, 32 filters of size 3x3
            32, (3, 3), activation="relu"
        ),
        tf.keras.layers.MaxPooling2D(pool_size=(2, 2)),     # 2x2 max pooling
        tf.keras.layers.Flatten(),   # flatten output of conv layers
